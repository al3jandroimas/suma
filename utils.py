...
Original file is located at
    https://colab.research.google.com/drive/1qbYGHaOflCpwSuDdHLAp8CRQd_bpMXzy
"""
...
def load_mnist():
    X_train = load_mnist_images('/content/train-images-idx3-ubyte.gz')
    y_train = load_mnist_labels('/content/train-labels-idx1-ubyte.gz')
    X_test = load_mnist_images('/content/t10k-images-idx3-ubyte.gz')
    y_test = load_mnist_labels('/content/t10k-labels-idx1-ubyte.gz')
...
def load_omniglot():
    x_train = np.load('/content/omniglot_x_train.npy')
    y_train = np.load('/content/omniglot_y_train.npy')

    x_test = np.load('/content/omniglot_x_test.npy')
    y_test = np.load('/content/omniglot_y_test.npy')

    valid_classes = np.load('/content/omniglot_valid_classes.npy')
...
def load_omniglot_vinyals():
    ...
    """
    with open('/content/train_vinyals_aug90.pkl', 'rb') as f:
        data = pkl.load(f, encoding='bytes')
        x_train = data[b'images']
        y_train = data[b'labels']
        y_str_train = data[b'label_str']

    with open('/content/val_vinyals_aug90.pkl', 'rb') as f:
        data = pkl.load(f, encoding='bytes')
        x_val = data[b'images']
        y_val = data[b'labels']
    ...
    with open('/content/test_vinyals_aug90.pkl', 'rb') as f:
        data = pkl.load(f, encoding='bytes')
...
def process_omniglot_vinyals_split():
    ...
    images_dirpath_train = '/content/images_background/'
    images_dirpath_test = '/content/images_evaluation/'
    ...
    for a_dir in alphabets_dirs:
        ...
        for c_dir in chars_dirs:
            ...
            for i_path in img_paths:
                ...
                if c_dir.replace('/content/images_background/', '')[:-1] in y_str_train_new \
                        or c_dir.replace('/content/images_evaluation/', '')[:-1] in y_str_train_new:
                    print('train:', c_dir)
    ...
    np.save('/content/omniglot_valid_classes', valid_classes)
    ...
    np.save('/content/omniglot_x_train', x_train)
    np.save('/content/omniglot_y_train', y_train)
    ...
    np.save('/content/omniglot_x_test', x_test)
    np.save('/content/omniglot_y_test', y_test)
