...
Original file is located at
    https://colab.research.google.com/drive/1vpb7stOjLdaXkmriNv_L1-gF1AAT8s3t
"""
...
config = importlib.import_module('%s.%s' % ('suma', 'bn2_omniglot_tp'))
if not 0:
    experiment_id = '%s-%s' % ('bn2_omniglot_tp', time.strftime("%Y_%m_%d", time.localtime()))
    utils.autodir('metadata')
    save_dir = 'metadata/' + experiment_id
    utils.autodir(save_dir)
else:
    save_dir = utils.find_model_metadata('metadata/', 'bn2_omniglot_tp')
...
# logs
utils.autodir('logs')
sys.stdout = logging.Logger('logs/%s.log' % experiment_id)
...
for i in range(2):
    xs.append(tf.placeholder(tf.float32, shape=(config.batch_size / 2,) + config.obs_shape))
...
with tf.Session() as sess:
    ...
    for iteration, x_batch in zip(batch_idxs, train_data_iter.generate()):
        ...
        if 0 and iteration < last_iteration:
        ...
        if iteration == 0:
            ...
            xfs = np.split(x_batch, 2)
            feed_dict = {tf_lr: lr, tf_student_grad_scale: student_grad_scale}
            feed_dict.update({xs[i]: xfs[i] for i in range(2)})
